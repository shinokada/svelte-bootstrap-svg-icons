...
const title = 'Getting Started - Svelte Bootstrap SVG Icons v1';
const description = 'How to get started with Svelte Bootstrap SVG Icons v1';
...
test('Guide Svelte 4 getting started page has expected h1, meta title', async ({ page }) => {
  await expect(page.getByRole('heading', { name: title })).toBeVisible();
});

test('Guide Svelte 4 getting started page has expected meta title', async ({ page }) => {
  await expect(page).toHaveTitle(title);
});

test('Guide Svelte 4 getting started page has expected meta description', async ({ page }) => {
  const metaDescription = page.locator('meta[name="description"]');
  await expect(metaDescription).toHaveAttribute('content', description);
});

test('Guide Svelte 4 getting started page has expected meta og', async ({ page, baseURL }) => {
  ...
  const url = baseURL + '/guide/svelte-4/getting-started';
  await expect(metaOgUrl).toHaveAttribute('content', url);
  const metaOgImage = page.locator('meta[property="og:image"]');
  await expect(metaOgImage).toHaveAttribute('content', imgUrl);
});

test('Guide Svelte 4 getting started page has expected meta twitter', async ({ page }) => {
